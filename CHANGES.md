### RQ 3.0 (unreleased)
* `TimerDeathPenalty` no longer replaces `JobTimeoutException.__init__`. Each penalty uses its own subclass so other `JobTimeoutException` messages stay intact.
