...
class TestThreadPoolWorker(RQTestCase):
    ...
    def test_create_worker(self):
        ...
        # With single string argument
        w = ThreadPoolWorker('foo')
        self.assertEqual(w.queues[0].name, 'foo')

        # With list of strings
        w = ThreadPoolWorker(['foo', 'bar'])
        self.assertEqual(w.queues[0].name, 'foo')
        self.assertEqual(w.queues[1].name, 'bar')
        ...
        # With iterable of strings
        w = ThreadPoolWorker(iter(['foo', 'bar']))
        self.assertEqual(w.queues[0].name, 'foo')
        self.assertEqual(w.queues[1].name, 'bar')

        # With single Queue
        w = ThreadPoolWorker(Queue('foo'))
        self.assertEqual(w.queues[0].name, 'foo')

        # With iterable of Queues
        w = ThreadPoolWorker(iter([Queue('foo'), Queue('bar')]))
        self.assertEqual(w.queues[0].name, 'foo')
        self.assertEqual(w.queues[1].name, 'bar')

        # With list of Queues
        w = ThreadPoolWorker([Queue('foo'), Queue('bar')])
        self.assertEqual(w.queues[0].name, 'foo')
        self.assertEqual(w.queues[1].name, 'bar')

        # With string and serializer
        w = ThreadPoolWorker('foo', serializer=json)
        self.assertEqual(w.queues[0].name, 'foo')

        # With queue having serializer
        w = ThreadPoolWorker(Queue('foo'), serializer=json)
        self.assertEqual(w.queues[0].name, 'foo')

    def test_work_and_quit(self):
        """ThreadPoolWorker processes work, then quits."""
        fooq, barq = Queue('foo'), Queue('bar')
        w = ThreadPoolWorker([fooq, barq])
        self.assertEqual(
            w.work(burst=True), False,
            'Did not expect any work on the queue.'
        )
    ...
    def test_work_and_quit_custom_serializer(self):
        """ThreadPoolWorker processes work, then quits."""
        fooq, barq = Queue('foo', serializer=JSONSerializer), Queue('bar', serializer=JSONSerializer)
        w = ThreadPoolWorker([fooq, barq], serializer=JSONSerializer)
        self.assertEqual(
            w.work(burst=True), False,
            'Did not expect any work on the queue.'
        )
    ...
    def test_worker_all(self):
        """ThreadPoolWorker.all() works properly"""
        foo_queue = Queue('foo')
        bar_queue = Queue('bar')

        w1 = ThreadPoolWorker([foo_queue, bar_queue], name='w1')
        w1.register_birth()
        w2 = ThreadPoolWorker([foo_queue], name='w2')
        w2.register_birth()

        self.assertEqual(
            set(ThreadPoolWorker.all(connection=foo_queue.connection)),
            set([w1, w2])
        )
        self.assertEqual(set(ThreadPoolWorker.all(queue=foo_queue)), set([w1, w2]))
        self.assertEqual(set(ThreadPoolWorker.all(queue=bar_queue)), set([w1]))

        w1.register_death()
        w2.register_death()

    def test_worked_in_parallel(self):
        """Test that each job sleeps in a different thread"""
        q = Queue()
        q.enqueue(long_running_job, 1)
        q.enqueue(long_running_job, 1)
        q.enqueue(long_running_job, 1)
        q.enqueue(long_running_job, 1)
        t0 = datetime.now()
        w = ThreadPoolWorker([q], pool_size=4)
        w.work(burst=True)
        t1 = datetime.now()
        self.assertGreaterEqual((t1 - t0).seconds, 1)
        self.assertLess((t1 - t0).seconds, 1.1)

    def test_find_by_key(self):
        """ThreadPoolWorker.find_by_key restores queues, state and job_id."""
        queues = [Queue('foo'), Queue('bar')]
        w = ThreadPoolWorker(queues)
        w.register_death()
        w.register_birth()
        w.set_state(WorkerStatus.STARTED)
        worker = ThreadPoolWorker.find_by_key(w.key)
        self.assertEqual(worker.queues, queues)
        self.assertEqual(worker.get_state(), WorkerStatus.STARTED)
        self.assertEqual(worker._job_id, None)
        self.assertTrue(worker.key in ThreadPoolWorker.all_keys(worker.connection))
        self.assertEqual(worker.version, VERSION)

        # If worker is gone, its keys should also be removed
        worker.connection.delete(worker.key)
        ThreadPoolWorker.find_by_key(worker.key)
        self.assertFalse(worker.key in ThreadPoolWorker.all_keys(worker.connection))

        self.assertRaises(ValueError, ThreadPoolWorker.find_by_key, 'foo')

    def test_worker_ttl(self):
        """ThreadPoolWorker ttl."""
        w = ThreadPoolWorker([])
        w.register_birth()
        [worker_key] = self.testconn.smembers(ThreadPoolWorker.redis_workers_keys)
        self.assertIsNotNone(self.testconn.ttl(worker_key))
        w.register_death()

    def test_work_via_string_argument(self):
        """ThreadPoolWorker processes work fed via string arguments."""
        q = Queue('foo')
    ...
    def test_heartbeat(self):
        ...
        self.assertIsNotNone(self.testconn.hget(w.key, 'birth'))
        self.assertTrue(last_heartbeat is not None)
        w = ThreadPoolWorker.find_by_key(w.key)
        self.assertIsInstance(w.last_heartbeat, datetime)
    ...
    def test_maintain_heartbeats(self):
        """worker.maintain_heartbeats() shouldn't create new job keys"""
        queue = Queue(connection=self.testconn)
        worker = ThreadPoolWorker([queue], connection=self.testconn)
        job = queue.enqueue(say_hello)
        worker.maintain_heartbeats(job)
        self.assertTrue(self.testconn.exists(worker.key))
        self.assertTrue(self.testconn.exists(job.key))
    ...
    @slow
    def test_heartbeat_survives_lost_connection(self):
        with mock.patch.object(ThreadPoolWorker, 'heartbeat') as mocked:
            # None -> Heartbeat is first called before the job loop
            mocked.side_effect = [None, redis.exceptions.ConnectionError()]
            q = Queue()
            w = ThreadPoolWorker([q])
    ...
    def test_max_jobs(self):
        """ThreadPoolWorker exits after number of jobs complete."""
        queue = Queue()
        job1 = queue.enqueue(do_nothing)
        job2 = queue.enqueue(do_nothing)
        worker = ThreadPoolWorker([queue])
    ...
    def test_disable_default_exception_handler(self):
        ...
        job = queue.enqueue(div_by_zero)
        worker = ThreadPoolWorker([queue], disable_default_exception_handler=False)
        worker.work(burst=True)
        ...
        # disable_default_exception_handler is True
        job = queue.enqueue(div_by_zero)
        worker = ThreadPoolWorker([queue], disable_default_exception_handler=True)
        worker.work(burst=True)
        self.assertFalse(job in registry)
    ...
    def test_custom_exc_handling(self):
        ...
        w = ThreadPoolWorker([q], exception_handlers=first_handler)
        w.work(burst=True)
        ...
        job = q.enqueue(div_by_zero)
        w = ThreadPoolWorker([q], exception_handlers=[first_handler, second_handler])
        w.work(burst=True)
        ...
        job = q.enqueue(div_by_zero)
        w = ThreadPoolWorker([q], exception_handlers=[first_handler, black_hole,
                                            second_handler])
        w.work(burst=True)
    ...
    @slow  # noqa
    def test_timeouts(self):
        """ThreadPoolWorker kills jobs after timeout."""
        sentinel_file = '/tmp/.rq_sentinel'
    ...
    def test_worker_sets_result_ttl(self):
        """Ensure that ThreadPoolWorker properly sets result_ttl for individual jobs."""
        q = Queue()
        job = q.enqueue(say_hello, args=('Frank',), result_ttl=10)
        w = ThreadPoolWorker([q])
        self.assertIn(job.get_id().encode(), self.testconn.lrange(q.key, 0, -1))
    ...
    def test_custom_job_class(self):
        """Ensure ThreadPoolWorker accepts custom job class."""
        q = Queue()
        worker = ThreadPoolWorker([q], job_class=CustomJob)
        self.assertEqual(worker.job_class, CustomJob)

    def test_custom_queue_class(self):
        """Ensure ThreadPoolWorker accepts custom queue class."""
        q = CustomQueue()
        worker = ThreadPoolWorker([q], queue_class=CustomQueue)
        self.assertEqual(worker.queue_class, CustomQueue)

    def test_custom_queue_class_is_not_global(self):
        """Ensure ThreadPoolWorker custom queue class is not global."""
        q = CustomQueue()
        worker_custom = ThreadPoolWorker([q], queue_class=CustomQueue)
        q_generic = Queue()
        worker_generic = ThreadPoolWorker([q_generic])
        self.assertEqual(worker_custom.queue_class, CustomQueue)
        self.assertEqual(worker_generic.queue_class, Queue)
        self.assertEqual(ThreadPoolWorker.queue_class, Queue)

    def test_custom_job_class_is_not_global(self):
        """Ensure ThreadPoolWorker custom job class is not global."""
        q = Queue()
        worker_custom = ThreadPoolWorker([q], job_class=CustomJob)
        q_generic = Queue()
        worker_generic = ThreadPoolWorker([q_generic])
        self.assertEqual(worker_custom.job_class, CustomJob)
        self.assertEqual(worker_generic.job_class, Job)
        self.assertEqual(ThreadPoolWorker.job_class, Job)
    ...
    def test_work_unicode_friendly(self):
        """ThreadPoolWorker processes work with unicode description, then quits."""
        q = Queue('foo')
        w = ThreadPoolWorker([q])
        job = q.enqueue('tests.fixtures.say_hello', name='Adam',
                        description='你好 世界!')
    ...
    def test_work_log_unicode_friendly(self):
        """ThreadPoolWorker process work with unicode or str other than pure ascii content,
        logging work properly"""
        q = Queue("foo")
        w = ThreadPoolWorker([q])
    ...
    def test_suspend_worker_execution(self):
        """Test Pause ThreadPoolWorker Execution"""
    ...
    def test_worker_hash_(self):
        """ThreadPoolWorkers are hashed by their .name attribute"""
        q = Queue('foo')
        w1 = ThreadPoolWorker([q], name="worker1")
        w2 = ThreadPoolWorker([q], name="worker2")
        w3 = ThreadPoolWorker([q], name="worker1")
        worker_set = set([w1, w2, w3])
        self.assertEqual(len(worker_set), 2)
    ...
    def test_should_run_maintenance_tasks(self):
        """ThreadPoolWorkers should run maintenance tasks on startup and every hour."""
        queue = Queue(connection=self.testconn)
        worker = ThreadPoolWorker(queue)
        self.assertTrue(worker.should_run_maintenance_tasks)
    ...
    def test_worker_calls_clean_registries(self):
        """ThreadPoolWorker calls clean_registries when run."""
        queue = Queue(connection=self.testconn)
        registry = StartedJobRegistry(connection=self.testconn)
        self.testconn.zadd(registry.key, {'foo': 1})
    ...
    def test_job_dependency_race_condition(self):
        ...
        q = Queue()
        w = ThreadPoolWorker([q])
        with mock.patch.object(ThreadPoolWorker, 'execute_job', wraps=w.execute_job) as mocked:
            parent_job = q.enqueue(say_hello, result_ttl=0)
            Queue._add_enqueue = parent_job
            job = q.enqueue_call(say_hello, depends_on=parent_job)
            w.work(burst=True)
    ...
    def test_log_job_description_false(self, mock_logger_info):
        """Check that log_job_description False causes job lifespan to not be logged."""
        q = Queue()
        w = ThreadPoolWorker([q], log_job_description=False)
        q.enqueue(say_hello, args=('Frank',), result_ttl=10)
        w.dequeue_job_and_maintain_ttl(10)
        self.assertNotIn("Frank", mock_logger_info.call_args[0][2])
    ...
    def test_python_version(self):
        ...
        python_version = 'X.Y.Z.final'  # dummy version
        self.assertNotEqual(python_version, sys.version)  # otherwise tests are pointless
        w2 = ThreadPoolWorker([q])
        w2.python_version = python_version
        w2.register_birth()
        self.assertEqual(w2.python_version, python_version)
        # making sure that version is preserved when worker is retrieved by key
        worker = ThreadPoolWorker.find_by_key(w2.key)
        self.assertEqual(worker.python_version, python_version)
