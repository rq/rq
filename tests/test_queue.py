...
from tests import RQTestClusterCase
...
class TestQueueCluster(RQTestClusterCase):
    ...
    def test_all_queues(self):
        ...
        w = Worker([q2, q3])
        """
        rq in redis cluster mode not support watch method, 
        w.work() will add failed task to queue, skip untill 
        redis-py-cluster support watch method
        """
        # w.work(burst=True)
    ...
    # skip this test because redis-py-cluster does not support watch method
    # def test_enqueue_dependents(self):
    #     """Enqueueing dependent jobs pushes all jobs in the depends set to the queue
    #     and removes them from DeferredJobQueue."""
    #     q = Queue()
    ...
    def test_redis_mode(self):
        ...
        logger.info('redis mode is {}'.format(ret))
        self.assertEqual(ret, 'cluster')

    def test_is_redis_cluster(self):
        q = Queue()
        ret = q.is_redis_cluster(self.testconn)
        logger.info('is redis cluster: {}'.format(ret))
