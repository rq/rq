...
def ensure_job_list(obj):
    ...
    return (
        list(obj) if isinstance(obj, Iterable) and not isinstance(obj, str) else [obj]
    )
...
def split_list(
    a_list: Sequence[_T], segment_size: int
...
def get_call_string(
    ...
    list_kwargs = [
        "{0}={1}".format(k, as_text(truncate_long_string(repr(v), max_length)))
...
def get_connection_from_queues(
    queues_or_names: Iterable[Union[str, "Queue"]],
) -> Optional["Redis"]:
...
def get_next_scheduled_time(cron_string: str, use_local_timezone: bool = False) -> datetime.datetime:
    ...
    try:
        ...
        warnings.warn(
            "croniter package not installed. Please install it with: pip install croniter",
            ImportWarning,
        )
        ...
        if use_local_timezone:
            try:
                ...
                warnings.warn(
                    "tzlocal package not installed. Using UTC timezone.", ImportWarning
                )
