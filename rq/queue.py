...
class Queue(object):
    ...
    def batch(self):
        return Batch(name=self.name, default_timeout=self._default_timeout,
                     connection=self.connection, async=self._async, job_class=self.job_class)
...
class Batch(Queue):
    def __init__(self, name='default', default_timeout=None, connection=None,
                 async=True, job_class=None):
        super(Batch, self).__init__(name=name, default_timeout=default_timeout,
                                    connection=connection, async=async, job_class=job_class)
        self.pipe = self.connection._pipeline()

    def enqueue_job(self, job, pipeline=None, at_front=False):
        # ignoring the passed in pipeline in favor of the one from the batch
        return super(Batch, self).enqueue_job(job, self.pipe, at_front)
