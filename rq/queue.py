...
class Queue(object):
    ...
    def lpop(cls, queue_keys, timeout, connection=None):
        ...
        if timeout and not cls.is_redis_cluster():  # blocking variant
            if timeout == 0:
    ...
    @classmethod
    def redis_mode(cls, connection=None):
        """Return redis mode
    ...
    @classmethod
    def is_redis_cluster(cls, connection=None):
        """Judge is redis in cluster mode
        :returns: true in cluster mode false or not
        :rtype: boolean
        """

        return cls.redis_mode(connection) == 'cluster'
