...
class Queue(object):
    ...
    def lpop(cls, queue_keys, timeout, connection=None):
        ...
        if timeout and not self.is_redis_cluster():  # blocking variant
            if timeout == 0:
    ...
    def redis_mode(self, connection=None):
        """Return redis mode
        :returns: redis mode, cluster(cluster mode) or standalone(single instance)
        :rtype: str
        """
        connection = resolve_connection(connection)
        ...
        if 'redis_mode' in info:
            ...
            node_url, node_info = info.popitem()
            return node_info.get('redis_mode')

    def is_redis_cluster(self, connection=None):
        """Judge is redis in cluster mode
        :returns: true in cluster mode false or not
        :rtype: boolean
        """

        return self.redis_mode(connection) == 'cluster'
