...
class Queue(object):
                ...
                while True:
                    try:
                        ...
                        if depends_on.get_status() != Status.FINISHED:
                            ...
                            if depends_on.origin != self.name:
                                job.meta['queue_params'] = self.dump()
                            job.save()
                            return job
                        break
                    except WatchError:
    ...
    def enqueue_dependents(self, job):
        ...
        while True:
            ...
            queue = self
            queue_name = dependent.origin or self.name
            if queue_name != self.name:
                params = dependent.meta.get('queue_params', {})
                queue = Queue(connection=self.connection, **params)
    ...
    def dump(self):
        """Dumps Queue constructor serializable kwargs to dictionary"""
        return {'name': self.name, 'default_timeout': self._default_timeout,
                'async': self._async, 'job_class': self.job_class}
