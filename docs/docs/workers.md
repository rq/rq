...
* `get_current_executions()` - the executions it's currently running, read from Redis so it also works on workers retrieved via `Worker.all()`
...
* `current_execution_count` - number of executions it's currently handling, without the cost of fetching them
