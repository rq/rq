...
### Queues with Round-Robin and Random access

The subclasses of `Queue`, `RoundRobinQueue` and `RandomQueue` 
implement round-robin and random access to a list of queues. 
...
qs = [Queue(f'q{i}', connection=redis) for i in range(5)]

for i in range(5):
    for j in range(3):
        qs[i].enqueue(say_hello, f'queue={i}, job={j}')

for k in range(15):
    job, queue = Queue.dequeue_any(qs, None, connection=redis)
    print(job.args[0])
```
Since the queues are polled in order, the ordering of the dequeued jobs is:
```
queue=0, job=0
queue=0, job=1
queue=0, job=2
...
Sometimes you may want to use a Round-Robin strategy, so that all 
queues are treated equally (at the first iteration the most preferent queue is the first one, but in the second 
iteration it is the second queue, and so on). Consider the modified script:
...
qs = [RoundRobinQueue(f'q{i}', connection=redis) for i in range(5)]
...
for k in range(15):
    job, queue = RoundRobinQueue.dequeue_any(qs, None, connection=redis)
    print(job.args[0])
```
Now the jobs are dequeued following a round-robin strategy:
```
queue=0,job=0
queue=1,job=0
queue=2,job=0
...
If instead of using `RoundRobinQueue`, it is used `RandomQueue`, then at each iteration the queues are
shuffled, so that the ordering of access to them is random, and a
run of the script may give:
```
queue=3, job=0
queue=0, job=0
queue=2, job=0
